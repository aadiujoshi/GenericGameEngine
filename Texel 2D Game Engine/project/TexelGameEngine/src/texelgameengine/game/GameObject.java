...
public class GameObject extends PaintableObject implements GameUpdateListener
{
    ...
    private final boolean CHUNK_BUILDER_DETERMINES_X_Y;
    ...
    public GameObject(float x, float y, float width, float height, BufferedImage texture){
        ...
        this.CHUNK_BUILDER_DETERMINES_X_Y = false;
    }

    
    public GameObject(float x, float y, float width, float height, String textureFilename) 
    {
        super(TextureResourceLoader.getTexture(textureFilename));
    ...
    /** 
     * x and y are determined by chunkbuilder and or gamemap validation
    */
    public GameObject(float width, float height, String textureFilepath) {
        super(TextureResourceLoader.getTexture(textureFilepath));
        this.tags = new GameTags();
        this.width = width;
        this.height = height;
        this.hitboxEnabled = true;
        ...
        this.INIT_TIME = System.currentTimeMillis();
        this.OBJECT_HASH_CODE = GameWorld.getNextFreeHashCode();
        this.CHUNK_BUILDER_DETERMINES_X_Y = true;
    }
    ...
    public void paintObject(GraphicsEvent e) {
        ...
        Point p = GameWorld.toScreenSpace((int)x, (int)y, e);
        
        g.drawImage(getTexture(), null, (int)p.getX()-(getTexture().getWidth()/2), (int)p.getY()-(getTexture().getHeight()/2));
    }
    ...
    public boolean chunkBuilderDeterminesXY() { return CHUNK_BUILDER_DETERMINES_X_Y; }
